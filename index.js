...
console.log(MenuElemaniOlustur("karışık tost", 3, "tostlar"));
/*
...
const burger = {
  ...
  indirim: function (indirim) {
    if (indirim === "öğretmen" || indirim === "öğrenci") {
...
*/
console.log((degerlendirmeler["isim"] = "Ahmet"));
/*  Görev 4 (ototest yok):  
...
/*  Görev 7:  
	degerlendirmelerden en son değerlendirmeyi döndüren adı `SonDegerlendirmeyiAl` olan bir fonksiyon yazın 
...
function SonDegerlendirmeyiAl(degerlendirmeler) {
  let { isim, puan, geribildirim } =
    degerlendirmeler[degerlendirmeler.length - 1];
  return (
    isim +
    " isimli kişi " +
    puan +
...
function PuanaGoreDegerlendirmeAl(degerlendirmeler, puan) {
  ...
  for (let i = 0; i < degerlendirmeler.length; i++) {
    if (
      degerlendirmeler[i].puan >= puan &&
      degerlendirmeler[i].puan < puan + 1
    ) {
      newArray.push(degerlendirmeler[i]);
    }
  }
  return newArray;
}
...
function UzunDegerlendirmeleriAl(degerlendirmeler, sayı) {
  let yeniDizi = degerlendirmeler.filter(
    element => element.geribildirim.split(" ").length > sayı
  );
